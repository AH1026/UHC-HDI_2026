...
econ_labels <- c("1"="Low income","2"="Lower-middle income","3"="Upper-middle income","4"="High income")
region_labels <- c("1"="EMR","2"="EUR","3"="AFR","4"="AMR","5"="WPR","6"="SEAR")
...
fit_ratio_vec <- function(outcome_var,
  ...
  if (adjusted) dat <- dat %>% filter(!is.na(control_cor), !is.na(age65), !is.na(tfr), !is.na(urban))
...
out_xlsx <- file.path(out_dir, "Table2_GEE_MeanRatios_Income_Region.xlsx")
