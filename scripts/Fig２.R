...
need_cols <- c("country","year","Region","UHC SCI","RMNCH","ID","NCD","SCA","HDI")
...
normalize_region <- function(x) {
  ...
  dplyr::case_when(
    xx %in% c("1","2","3","4","5","6") ~ dplyr::case_when(
      xx == "1" ~ "EMR",
      xx == "2" ~ "EUR",
      xx == "3" ~ "AFR",
      xx == "4" ~ "AMR",
    ...
    xx %in% c("EMR","EUR","AFR","AMR","WPR","SEAR") ~ xx,
    TRUE ~ NA_character_
  )
}
...
df2 <- df %>%
  ...
  mutate(
    ...
    region = factor(region, levels = c("EMR","EUR","AFR","AMR","WPR","SEAR"))
  ) %>%
...
dat_long <- df2 %>%
  select(year, region, all_of(ind_levels)) %>%
...
plot_panel <- function(sum_df_one, panel_label, show_legend, show_axis_titles) {
  ...
  p <- ggplot() +
    ...
    theme(
      ...
      axis.text.y.right = element_text(size = BASE - 1, angle = 0),
...
p_emr <- plot_panel(region_sum %>% filter(region == "EMR"), "EMR", TRUE, TRUE)
p_eur <- plot_panel(region_sum %>% filter(region == "EUR"), "EUR", FALSE, FALSE)
p_afr <- plot_panel(region_sum %>% filter(region == "AFR"), "AFR", FALSE, FALSE)
p_amr <- plot_panel(region_sum %>% filter(region == "AMR"), "AMR", FALSE, FALSE)
p_wpr <- plot_panel(region_sum %>% filter(region == "WPR"), "WPR", FALSE, FALSE)
p_sea <- plot_panel(region_sum %>% filter(region == "SEAR"), "SEAR", FALSE, FALSE)

fig_region6 <- (p_emr + p_eur) / (p_afr + p_amr) / (p_wpr + p_sea) +
...
ggsave(
  filename = file.path(out_dir, "Fig2_Trends_Region.tiff"),
  plot = fig_region6,
  device = "tiff",
  dpi = 300,
  width = 12,
...
openxlsx::addWorksheet(wb, "Region_medians")
region_excel <- region_sum %>%
  mutate(Group = as.character(region), Indicator = as.character(Indicator)) %>%
  select(Group, year, Indicator, n, Median)

openxlsx::writeData(wb, "Region_medians", region_excel)
openxlsx::saveWorkbook(wb, file = file.path(out_dir, "Fig2_Data_Region.xlsx"), overwrite = TRUE)
