...
sub_outcomes <- list(
  list(col="RMNCH", label="RMNCH", tiff="SuppFig1_GEE_YearSlopes_RMNCH.tiff"),
  list(col="ID",    label="ID",    tiff="SuppFig2_GEE_YearSlopes_ID.tiff"),
  list(col="NCD",   label="NCD",   tiff="SuppFig3_GEE_YearSlopes_NCD.tiff"),
  list(col="SCA",   label="SCA",   tiff="SuppFig4_GEE_YearSlopes_SCA.tiff")
)
...
for(s in sub_outcomes){
  if(!(s$col %in% names(raw))) stop("Required sub-index column not found: ", s$col)
  out <- make_2x2_and_save(s$col, s$label, y_lim_sub, s$tiff)
...
excel_out <- file.path(out_dir, "SuppFigs_GEE_YearSlopes_Subindices.xlsx")
