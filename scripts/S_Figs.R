...
out_dir <- "output：Supporting Figures"
...
estimate_yearly_slope <- function(outcome_col, outcome_label, expo_col, expo_label) {
  dat <- raw %>%
    ...
    dplyr::mutate(
      ...
      region_nom = factor(normalize_code(region_raw), levels = region_levels),
      region_nom = stats::relevel(region_nom, ref = "2")
    ) %>%
    dplyr::filter(
      !is.na(y), !is.na(expo), !is.na(region_nom),
  ...
  fit <- geepack::geeglm(
    y ~ expo * year_f + region_nom + corruption + elderly + tfr + urban,
    id = country,
    waves = year,
    data = dat,
...
make_2x2_and_save <- function(outcome_col, outcome_label, y_lim, tiff_filename) {
  ...
  tiff_out <- file.path(out_dir, tiff_filename)
  ggsave(
    filename = tiff_out,
    plot = p_2x2,
    width = 12,
  ...
  list(
    tiff = tiff_out,
    results = dplyr::bind_rows(df_hdi, df_gni, df_edu, df_leix)
  )
}

figs <- list(
  list(col = "UHC SCI", label = "UHC SCI", y_lim = c(0, 10), tiff = "S1Fig.tiff"),
  list(col = "RMNCH", label = "RMNCH", y_lim = c(-2, 18), tiff = "S2Fig.tiff"),
  list(col = "ID", label = "ID", y_lim = c(-2, 18), tiff = "S3Fig.tiff"),
  list(col = "NCD", label = "NCD", y_lim = c(-2, 18), tiff = "S4Fig.tiff"),
  list(col = "SCA", label = "SCA", y_lim = c(-2, 18), tiff = "S5Fig.tiff")
)
...
for (s in figs) {
  if (!(s$col %in% names(raw))) stop("Required outcome column not found: ", s$col)
  out <- make_2x2_and_save(s$col, s$label, s$y_lim, s$tiff)
  all_res[[s$label]] <- out$results
  saved_files <- c(saved_files, out$tiff)
}

all_df <- dplyr::bind_rows(all_res)

excel_out <- file.path(out_dir, "S1-5Figs_All.xlsx")

if (file.exists(excel_out)) {
  ok_remove <- tryCatch(file.remove(excel_out), error = function(e) FALSE, warning = function(w) FALSE)
  if (!isTRUE(ok_remove) && file.exists(excel_out)) {
...
for (s in figs) {
  ...
  openxlsx::writeData(wb, nm, dplyr::filter(all_df, outcome == nm))
}
