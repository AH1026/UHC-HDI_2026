...
out_dir <- "UHC×HDI論文_output：Table１"
...
if (!("year" %in% names(df_long))) stop("year 列が見つかりません。列名: ", paste(names(df_long), collapse = ", "))
...
if (length(missing_cols) > 0) stop("データに存在しない列があります: ", paste(missing_cols, collapse = ", "))
...
if (any(is.na(long2$digits))) {
  stop("digits_map に無い Indicator が含まれています: ",
...
openxlsx::addWorksheet(wb, "Table1_記述統計")
...
openxlsx::writeData(wb, "Table1_記述統計", x = as.data.frame(t(header1)), startRow = 2, colNames = FALSE)
openxlsx::writeData(wb, "Table1_記述統計", x = as.data.frame(t(header2)), startRow = 3, colNames = FALSE)
openxlsx::writeData(wb, "Table1_記述統計", x = out, startRow = 4, colNames = FALSE)

for (i in seq_along(years_use)) {
  c1 <- 3 + (i - 1) * 2
  c2 <- c1 + 1
  openxlsx::mergeCells(wb, "Table1_記述統計", cols = c1:c2, rows = 2)
}
...
openxlsx::addStyle(wb, "Table1_記述統計", style = style_head,  rows = 2, cols = 1:(2 + 2 * length(years_use)), gridExpand = TRUE)
openxlsx::addStyle(wb, "Table1_記述統計", style = style_head2, rows = 3, cols = 1:(2 + 2 * length(years_use)), gridExpand = TRUE)

openxlsx::setColWidths(wb, "Table1_記述統計", cols = 1, widths = 2)
openxlsx::setColWidths(wb, "Table1_記述統計", cols = 2, widths = 30)
openxlsx::setColWidths(wb, "Table1_記述統計", cols = 3:(2 + 2 * length(years_use)), widths = 16)

openxlsx::freezePane(wb, "Table1_記述統計", firstRow = TRUE)

out_file <- file.path(out_dir, "Table1_記述統計_UHC4sub_HDI3elem_covariates.xlsx")
