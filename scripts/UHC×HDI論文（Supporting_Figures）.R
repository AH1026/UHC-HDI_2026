...
out_dir <- "UHC×HDI論文_output：Supporting Figures"
...
make_2x2_and_save <- function(outcome_col, outcome_label, y_lim){
  ...
  tiff_out <- file.path(out_dir, paste0("AdjOnly_INDEP_", outcome_label, "_2x2.tiff"))
...
sub_outcomes <- list(
  list(col="RMNCH", label="RMNCH"),
  list(col="ID",    label="ID"),
  list(col="NCD",   label="NCD"),
  list(col="SCA",   label="SCA")
)
...
for(s in sub_outcomes){
  if(!(s$col %in% names(raw))) stop("サブ指標列が見つかりません: ", s$col)
  out <- make_2x2_and_save(s$col, s$label, y_lim_sub)
...
excel_out <- file.path(out_dir, "AdjOnly_INDEP_Subindices_results.xlsx")
