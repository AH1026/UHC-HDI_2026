...
covars_adjusted <- c("region_nom", "coc", "age65", "tfr", "urban")
...
out_plot <- file.path(out_dir, "Fig3.tiff")
ggsave(
  filename = out_plot,
  plot = p_all,
  device = "tiff",
...
xlsx_out <- file.path(out_dir, "Fig3.xlsx")
writexl::write_xlsx(
  list(
    "HDI" = o1$data,
    "GNI_index" = o2$data,
...
message("Saved: ", normalizePath(out_plot, winslash = "/", mustWork = FALSE))
message("Saved: ", normalizePath(xlsx_out, winslash = "/", mustWork = FALSE))