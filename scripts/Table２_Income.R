...
fit_income_vec <- function(outcome_var, levels_chr, ref_level, adjusted = TRUE, digits = 2) {
  dat0 <- df2 %>%
    mutate(y = as.numeric(.data[[outcome_var]])) %>%
    filter(!is.na(y), !is.na(country), !is.na(year), !is.na(income_group))
  
  if (adjusted) {
    dat0 <- dat0 %>%
      filter(!is.na(region_nom), !is.na(control_cor), !is.na(age65), !is.na(tfr), !is.na(urban))
  }
  ...
  fml <- if (adjusted) {
    y ~ income_group + region_nom + control_cor + age65 + tfr + urban + factor(year)
  } else {
    y ~ income_group + factor(year)
  }
  ...
  info <- dat %>%
    summarise(
      outcome = outcome_var,
      model = ifelse(adjusted, "Adjusted (Region + covariates)", "Crude"),
      family = "gaussian",
      link = "log",
      corstr = "independence",
      id_variable = "country",
      time_variable = "year",
      income_group_type = "ordinal (modeled as treatment-coded factor for pairwise contrasts)",
      region_type = "nominal",
...
income_adj <- data.frame(`Income group` = unname(econ_labels[econ_levels]), check.names = FALSE)
income_cru <- data.frame(`Income group` = unname(econ_labels[econ_levels]), check.names = FALSE)
info_list <- list()

for (v in outcomes) {
  r_adj <- fit_income_vec(v, econ_levels, "4", adjusted = TRUE, digits = 2)
  income_adj[[v]] <- unname(r_adj$vec[econ_levels])
  info_list[[paste0("Adj_", v)]] <- r_adj$info
  
  r_cru <- fit_income_vec(v, econ_levels, "4", adjusted = FALSE, digits = 2)
  income_cru[[v]] <- unname(r_cru$vec[econ_levels])
  info_list[[paste0("Crude_", v)]] <- r_cru$info
}

model_info <- dplyr::bind_rows(info_list)

out_xlsx <- file.path(out_dir, "Table2_Income.xlsx")
...
openxlsx::addWorksheet(wb, "Adjusted_Income_refHigh")
openxlsx::addWorksheet(wb, "Crude_Income_refHigh")
openxlsx::addWorksheet(wb, "ModelInfo")

openxlsx::writeData(wb, "Adjusted_Income_refHigh", income_adj)
openxlsx::writeData(wb, "Crude_Income_refHigh", income_cru)
openxlsx::writeData(wb, "ModelInfo", model_info)

openxlsx::setColWidths(wb, "Adjusted_Income_refHigh", cols = 1:ncol(income_adj), widths = "auto")
openxlsx::setColWidths(wb, "Crude_Income_refHigh", cols = 1:ncol(income_cru), widths = "auto")
openxlsx::setColWidths(wb, "ModelInfo", cols = 1:ncol(model_info), widths = "auto")

openxlsx::freezePane(wb, "Adjusted_Income_refHigh", firstRow = TRUE, firstCol = TRUE)
openxlsx::freezePane(wb, "Crude_Income_refHigh", firstRow = TRUE, firstCol = TRUE)
